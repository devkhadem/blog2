document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof window.posts !== 'undefined' && window.posts.length > 0) {
        // Filter for Lifestyle posts
        const lifestylePosts = window.posts.filter(post => post.category === 'Lifestyle');

        // Main Posts (Lifestyle posts)
        if(mainPostsContainer){
            mainPostsContainer.innerHTML = lifestylePosts.map(createPostCard).join('');
        }
